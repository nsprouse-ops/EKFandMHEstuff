...
def _mpc_loop(options):
    ...
    passed_disturbance_values = {"d_k": 1.0}
    ...
    for i in loop_iter:
        ...
        if mhe_result is not None:
            ...
            if bool(getattr(options, "rebuild_setpoints_on_disturbance_change", True)) and (mhe_result.M_eff >= current_mhe_window) and (
                abs(d_k_step_delta) >= 0.5
            ):
                options.steady_state_fixed_vars = {
                    "d_k": float(passed_disturbance_values["d_k"]),
                }
