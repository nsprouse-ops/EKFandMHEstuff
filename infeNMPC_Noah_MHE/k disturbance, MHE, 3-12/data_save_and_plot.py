...
def _plot_disturbance_sent(time_series, d_k_sent_hist, plant):
    """
    Plot the disturbance value sent from MHE to MPC over time.
    """
    figures = []
    names = []
    if d_k_sent_hist is None:
        return figures, names
    ...
    plt.xlabel(f"${plant.time_display_name[0]}$")
    plt.title("Disturbance Sent to MPC: d_k_sent")
    plt.grid(True)
    plt.legend()
    figures.append(fig)
    names.append("d_k_sent")
    return figures, names


def _handle_mpc_results(sim_data, time_series, io_data_array, plant, cpu_time, options, mhe_state_hist=None, true_state_hist=None, setpoint_values=None, d_k_sent_hist=None):
    """
    ...
    disturbance_figures, disturbance_names = _plot_disturbance_sent(time_series, d_k_sent_hist, plant)
    final_figures.extend(disturbance_figures)
    figure_names.extend(disturbance_names)
