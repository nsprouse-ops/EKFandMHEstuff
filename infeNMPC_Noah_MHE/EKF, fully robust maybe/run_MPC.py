...
def _mpc_loop(options):
    ...
    # Fix all disturbance states to nominal before any solve — prevents IPOPT from
    # exploiting them as free variables before the EKF has produced estimates.
    def _fix_disturbances(dist_vals):
        """dist_vals: dict {dist_name: value}."""
        blocks = [controller.finite_block, controller.infinite_block] if options.infinite_horizon else [controller]
        for blk in blocks:
            for dname in blk.Disturbance_index:
                val = dist_vals[dname] if isinstance(dist_vals, dict) else dist_vals
                for t in blk.time:
                    getattr(blk, dname)[t].fix(float(val))

    _ref_blk = controller.finite_block if options.infinite_horizon else controller
    _fix_disturbances({
        dname: pyo.value(getattr(_ref_blk, dname)[_ref_blk.time.first()])
        for dname in _ref_blk.Disturbance_index
    })
    ...
    dist_hist = {name: [] for name in ekf._dist_names}
    ...
    noise_seed = getattr(options, "measurement_noise_seed", None)
    noise_rng = np.random.default_rng(noise_seed)
    noise_vector = noise_rng.normal(loc=0.0, scale=1.0, size=options.num_horizons)
    ...
    for i in loop_iter:
        ...
        # EKF step: predict using applied MVs, update using all measured states
        u_applied = np.array(mv_row)
        y_meas = np.array([
            float(measured_now.get(name, ekf.xa[i]))
            for i, name in enumerate(ekf._state_names[:ekf._n_meas])
        ])
        ekf.step(u_applied, y_meas)
        unmeasured_xhat = ekf.get_unmeasured_xhat()

        # Feed EKF disturbance estimates back to controller
        dist_est = ekf.get_disturbance_estimates()
        print(f"  EKF disturbances [step {i+1}]: " + ", ".join(f"{k}={v:.4f}" for k, v in dist_est.items()))
        _fix_disturbances(dist_est)
        for dname, dval in dist_est.items():
            dist_hist[dname].append(dval)
        new_ss = _update_steady_state_for_disturbances(controller, options, dist_est)
        print(f"  Updated SS refs: " + ", ".join(f"{k}={v:.4f}" for k, v in new_ss.items()))
