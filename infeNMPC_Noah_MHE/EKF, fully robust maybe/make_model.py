...
def _make_steady_state_model(m, options, equations_module: str = "model_equations"):
    ...
    # Fix disturbance states to their nominal (initialize) value so the SS solve uses
    # the unperturbed model.  Without this, IPOPT treats them as free optimisation
    # variables and computes wrong steady-state MV references.
    if hasattr(m, 'Disturbance_index'):
        for dname in m.Disturbance_index:
            if hasattr(m, dname):
                var = getattr(m, dname)
                nominal = pyo.value(var[m.time.first()])
                for t in m.time:
                    var[t].fix(nominal)
...
def _make_infinite_horizon_model(m, options, equations_module: str = "model_equations"):
    ...
    if options.remove_collocation:
        ...
        for mv in m.finite_block.MV_index:
            getattr(m.finite_block,mv)[time0].fix()
...
def _update_steady_state_for_disturbances(controller, options, dist_est):
    """
    Re-solve the steady-state problem with the EKF disturbance estimates and
    ...
    options    : Options
    dist_est   : dict                {dist_name: value} from EKF disturbance estimates
    """
    m_ss = pyo.ConcreteModel()
    m_ss = _make_steady_state_model(m_ss, options)   # fixes disturbances to nominal

    # Override disturbance states with EKF estimates
    if hasattr(m_ss, 'Disturbance_index'):
        for dname in m_ss.Disturbance_index:
            if dname in dist_est and hasattr(m_ss, dname):
                var = getattr(m_ss, dname)
                for t in m_ss.time:
                    var[t].fix(float(dist_est[dname]))
