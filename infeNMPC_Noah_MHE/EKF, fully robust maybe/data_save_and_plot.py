...
def _plot_disturbances(time_series, dist_hist, plant):
    """
    Plot EKF disturbance estimates over time, one figure per disturbance.

    Args:
        time_series: Full list of time points (length N+1, starting at 0).
        dist_hist: Dict {dist_name: [values]}, one value per MPC step.
        plant: Plant model (for time axis label).

    Returns:
        figs: List of matplotlib figures.
        names: List of figure name strings.
    """
    figs, names = [], []
    for dname, hist in dist_hist.items():
        if not hist:
            continue
        t = time_series[1 : len(hist) + 1]
        fig = plt.figure()
        plt.plot(t, hist, label=f"EKF ${dname}$ estimate")
        plt.ylabel(f"${dname}$")
        plt.xlabel(f"${plant.time_display_name[0]}$")
        plt.title(f"EKF Disturbance Estimate: ${dname}$ vs. Time")
        plt.grid(True)
        plt.legend()
        figs.append(fig)
        names.append(f"{dname}_estimate")
    return figs, names


def _handle_mpc_results(sim_data, time_series, io_data_array, plant, cpu_time, options, est_state_hist=None, true_state_hist=None, setpoint_values=None, dist_hist=None):
    """
    ...
    if dist_hist:
        dist_figs, dist_names = _plot_disturbances(time_series, dist_hist, plant)
        final_figures.extend(dist_figs)
        figure_names.extend(dist_names)
